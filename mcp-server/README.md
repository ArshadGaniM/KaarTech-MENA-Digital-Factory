...
**`project`'s `projectId`, `projectName`, and `projectProfitCenterCode`**
are all manually entered — the first table since `resource`'s
`employeeId` with **no auto-generated business code at all**: unlike
`practice`/`department`/`team`/`position`, `project` has no `hasCode`
trigger, and `projectId` is caller-supplied and enforced unique at the DB
level the same way `resource`'s `employeeId` is (`add_project` with a
duplicate `projectId` fails with a 422 naming the field). None of the
